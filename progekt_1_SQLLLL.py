...
class Database:
    ...
    def create_tables(self):
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS delivery(id INTEGER PRIMARY KEY, user_id INTEGER,
    ...
    def get_cart_id(self, user_id):
        cart_id = self.cursor.execute(f"""SELECT cart.id FROM cart INNER JOIN users ON cart.user_id = users.id 
    ...
    def get_category(self):
        ...
        list_cat = []
        data = self.cursor.execute(f"""SELECT name FROM category """)
    ...
    def get_products(self, category):
        ...
        category_id = self.cursor.execute(f'SELECT id FROM category WHERE name = "{category}"')
        category_id = category_id.fetchone()[0]
        list_prod = []
        data = self.cursor.execute(f"""SELECT name FROM products WHERE category_id = {category_id} and status = 1""")
        for x in data.fetchall():
            for y in x:
                list_prod.append(y)
        return list_prod
    ...
    def add_user(self, vk_id, tg_id, name):
        ...
        if vk_id is None:
            ...
            tg_id = "null"
        user_id = self.cursor.execute(f"""SELECT id FROM users WHERE vk_id ={vk_id} or tg_id={tg_id}""").fetchone()[0]
        self.cursor.execute("""INSERT INTO cart (user_id, total, cart_status) VALUES (?, ?, ?)""", (user_id, 0, 1))
        self.connection.commit()
    ...
    def add_delivery(self, user_id, list1):
        ...
        if self.cursor.execute(f"""SELECT user_address, payment FROM delivery WHERE user_id ={id_user}""").fetchone() is None:
            self.cursor.execute('''INSERT INTO delivery (user_id, user_address, payment) VALUES (?, ?,  ?)''',
                        (id_user, list1[0], list1[1]))
            self.connection.commit()
        else:
            self.cursor.execute(f"""UPDATE delivery SET user_address = '{list1[0]}' WHERE user_id = {id_user}""")
            self.cursor.execute(f"""UPDATE delivery SET payment = {list1[1]} WHERE user_id = {id_user}""")
            self.connection.commit()
    ...
    def add_products_to_cart_row(self, user_id, product, amount):
        ...
        product_id = self.cursor.execute(f"SELECT id FROM products WHERE name = '{product}'").fetchone()[0]
        self.cursor.execute('''INSERT INTO cart_row (cart_id, product_id, amount) VALUES (?, ?, ?)''',
                    (cart_id, product_id, amount))
        self.connection.commit()
        new_total = self.cursor.execute(
        ...
        self.cursor.execute(f"""UPDATE cart SET total = {new_total} WHERE total = {total} and id = {cart_id}""")
        self.connection.commit()
    ...
    def del_cart_line(self, user_id, cart_row_id):
        ...
        total = self.cursor.execute(
            ...
            WHERE cart_status = 1 and (vk_id={user_id} or tg_id={user_id})""").fetchone()[0]
        cart_id = self.get_cart_id(user_id)
        amount = self.cursor.execute(f'SELECT amount FROM cart_row WHERE id ={cart_row_id}').fetchone()[0]
        ...
        self.cursor.execute(f"""DELETE FROM cart_row WHERE cart_id={cart_id} and id={cart_row_id}""")
        self.cursor.execute(f"""UPDATE cart SET total = {new_total} WHERE total ={total} and id = {cart_id}""")
        self.connection.commit()
    ...
    def delete_by_dish(self, user_id, product):
        ...
        cart_id = self.get_cart_id(user_id)
        product_id = self.cursor.execute(f"""SELECT id FROM products WHERE name = '{product}'""").fetchone()[0]
        amount = self.cursor.execute(f'SELECT amount FROM cart_row WHERE product_id ={product_id}').fetchone()[0]
        cart_row_id = \
        self.cursor.execute(f'SELECT id FROM cart_row WHERE product_id = {product_id} and cart_id={cart_id}').fetchone()[0]
        price = self.cursor.execute(f"SELECT price FROM products WHERE name = '{product}'").fetchone()[0]
        new_total = total - (amount * price)
        self.cursor.execute(f"""DELETE FROM cart_row WHERE cart_id={cart_id} and id={cart_row_id} """)
        self.connection.commit()
        self.cursor.execute(f"""UPDATE cart SET total = {new_total} WHERE total ={total} and id = {cart_id}""")
    ...
    def get_cart_row(self, user_id):
        ...
        total = self.cursor.execute(f"SELECT total From cart WHERE id = {cart_id}").fetchone()[0]
        return carts, total
    ...
    def add_to_order(self, user_id):
        ...
        cart_id = self.get_cart_id(user_id)
        status = 1
        delivery_id = self.cursor.execute(f"""SELECT delivery.id FROM delivery 
         INNER JOIN users ON delivery.user_id = users.id WHERE vk_id={user_id} or tg_id={user_id}""").fetchone()[0]
        now = datetime.now()
        max_time_to_cook = max(self.cursor.execute(
        ...
        id_user_in_users = \
        self.cursor.execute(f"""SELECT id FROM users WHERE vk_id ={user_id} or tg_id={user_id}""").fetchone()[0]
        self.cursor.execute("""INSERT INTO cart (user_id, total, cart_status) VALUES (?, ?, ?)""", (id_user_in_users, 0, 1))
        self.connection.commit()

    def get_orders(self, user_id):
        """ получение данных заказа"""
        orders = []
        cart_id_list = self.cursor.execute(f"""SELECT cart.id FROM cart INNER JOIN users ON cart.user_id = users.id 
        WHERE cart_status = 0 and (vk_id={user_id} or tg_id={user_id})""").fetchall()
        for x in cart_id_list:
            orders.append(self.cursor.execute(f"SELECT * FROM orders WHERE cart_id={x[0]} and status = 1").fetchone())
        return orders
    ...
    def change_order(self, user_id, order_id):
        ...
        cart_id = self.get_cart_id(user_id)
        self.cursor.execute(f"""DELETE FROM cart WHERE id ={cart_id}""")
        cart_id = self.cursor.execute(f"""SELECT cart_id FROM orders WHERE id = {order_id}""").fetchone()[0]
        self.cursor.execute(f"""UPDATE cart SET cart_status = 1 WHERE cart_status = 0 and id={cart_id}""")
        self.cursor.execute(f"""UPDATE cart_row SET status_in_order = 1 WHERE status_in_order = 0 and cart_id = {cart_id}""")
        self.cursor.execute(f"""DELETE FROM orders WHERE cart_id={cart_id} and id={order_id}""")
        self.connection.commit()

    def cancel_order(self, order_id):
        """ для отмены заказ нужен id заказа из таблицы заказов"""
        self.cursor.execute(f"""DELETE FROM orders WHERE id={order_id}""")
        self.connection.commit()
    ...
    def get_product_rating(self, product):
        ...
        product_id = self.cursor.execute(f"""SELECT id FROM products WHERE name = '{product}' """).fetchone()[0]
        product_rating = self.cursor.execute(
    ...
    def get_orders_rating(self):
        ...
        orders_rating = self.cursor.execute(f'SELECT * FROM orders_rating WHERE status = "принят"').fetchall()
        rating_list = []
        rating_texts = []
...
####################################################
    ...
    def adm_get_ord(self):
        ...
        orders = self.cursor.execute(f'SELECT * FROM orders WHERE status = 1').fetchall()
        return orders
    ...
    def get_user_category(self, user_id):
        ...
        try:
            cat = self.cursor.execute(f"SELECT category FROM users WHERE tg_id = {user_id}").fetchone()[0]
            return cat
        except:
    ...
    def adm_change_order_status(self, order_id):
        ...
        self.cursor.execute(f"UPDATE orders SET status = 0 WHERE status = 1 and id = {order_id} ")
        self.connection.commit()

    def adm_change_to_first_rank(self, user_id):
        """наделения правми админа 1го разряда"""
        self.cursor.execute(f"UPDATE users SET category = 1 WHERE (category = 0 or category = 2) and tg_id = {user_id} ")
        self.connection.commit()

    def adm_change_to_second_rank(self, user_id):
        """наделения правми админа 2го разряда"""
        self.cursor.execute(f"UPDATE users SET category = 2 WHERE (category = 0 or category = 1 )and tg_id = {user_id} ")
        self.connection.commit()

    def adm_get_rating_ord(self):
        """ выдача списка отзывов, которые на рассмотрении"""
        orders = self.cursor.execute(f'SELECT * FROM orders_rating WHERE status ="на рассмотрении"').fetchall()
        return orders

    def adm_get_ord_rewiew(self, order_id):
        """ выдача информации по отзыву"""
        review_mark = self.cursor.execute(f'SELECT rating, comment FROM orders_rating WHERE id ={order_id}').fetchone()
        return review_mark

    def adm_accept_stat_ord(self, ord_rat_id):
        self.cursor.execute(f"UPDATE orders_rating SET status='принят' WHERE id ={ord_rat_id}")
        self.connection.commit()

    def adm_refuse_stat_ord(self, ord_rat_id):
        self.cursor.execute(f"UPDATE orders_rating SET status='отклонен' WHERE id ={ord_rat_id}")
        self.connection.commit()

    def adm_get_rating_prod(self):
        """ выдача списка отзывов, которые на рассмотрении"""
        orders = self.cursor.execute(f'SELECT * FROM products_rating WHERE status ="на рассмотрении"').fetchall()
        return orders

    def adm_get_prod_rewiew(self, order_id):
        """ выдача информации по отзыву"""
        review_mark = self.cursor.execute(f'SELECT rating, comment FROM products_rating WHERE id ={order_id}').fetchone()
        return review_mark

    def adm_accept_stat_prod(self, ord_rat_id):
        self.cursor.execute(f"UPDATE products_rating SET status='принят' WHERE id ={ord_rat_id}")
        self.connection.commit()

    def adm_refuse_stat_prod(self, ord_rat_id):
        self.cursor.execute(f"UPDATE products_rating SET status='отклонен' WHERE id ={ord_rat_id}")
        self.connection.commit()

    def adm_get_products_status(self):
        list_of_prod = self.cursor.execute(f'SELECT name, status FROM products').fetchall()
        return list_of_prod

    def adm_stop_prod(self, product):
        self.cursor.execute(f'UPDATE products SET status = 0 WHERE name ="{product}"')
        self.connection.commit()

    def adm_unstop_prod(self, product):
        self.cursor.execute(f'UPDATE products SET status = 1 WHERE name ="{product}"')
        self.connection.commit()
    ...
    def adm_decrease_adm_rank(self, tg_id):
        self.cursor.execute(f'UPDATE users SET category = 0 WHERE tg_id = {tg_id}')
        self.connection.commit()
