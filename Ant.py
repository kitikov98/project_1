...
2. Также вы всегда можете найти актуальный перечень всех акций и предложений нажав кнопку : "таблица со всеми промокодами"
3. Чтобы всегда оставаться на связи, подпишитесь на нас в телеграмм канале, нажав кнопку : "Мы в Телеграме"
"""
    ...
    for event in longpoll.listen():
        ...
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            if event.obj.message['text'] in HI:
                ...
                bottext(text_inst, event.obj.message, keyboard_1)
                ...
                if event.from_user:
                    if event.obj.message['text'] == 'Меню блюд' or event.obj.message['text'] == "Меню!":
                        ...
                        bottext('Выбирай категорию', event.obj.message, menu_gener(menu_1, 0, 1))
                        ...
                        if event.obj.message['text'] in db.get_category():
                            ...
                            bottext('Выбирайте блюдо', event.obj.message,
                ...
                if user_stat_ind == 0:
                    ...
                    if event.obj.message['text'].isdigit():
                        ...
                        vk.messages.send(
                            ...
                            peer_id=event.obj.message['from_id'],
                            conversation_message_id=event.obj.message['from_id'],
                            message=send_discription(db.get_description_dish(event.obj.message['text'])[0]),
                            keyboard=keyboard_menu.get_keyboard(),
